...
public class AsrTts {
  ...
  /** Transform the semantic representation of the best hypothesis into
   *  a speech act event and return it.
   * @param utt
   * @return the speech act event corresponding to the semantic content in
   *         the best hypothesis
   *
  public static SpeechActEvent utterance2SpeechActEvent(UserTextFeedback utt) {
    // TODO: DO PARSING OF TEXT MESSAGE
    Map<String, String> tokens = new HashMap<>();
    SpeechActEvent sa = new SpeechActEvent();
    sa.arguments = new HashMap<String, String>();
    for (Map.Entry<String, String> keyVal : tokens.entrySet()) {
      switch (keyVal.getKey()) {
      case "speechact": sa.speechact = keyVal.getValue(); break;
      case "proposition": sa.proposition = keyVal.getValue(); break;
      case "sender": sa.sender = keyVal.getValue(); break;
      case "addressee": sa.addressee = keyVal.getValue(); break;
      case "id": sa.id = App.generateId(); break;
      default:
        sa.arguments.put(keyVal.getKey(), keyVal.getValue());
        break;
      }
    }
    if (!sa.isSetId()) {
      sa.setId(App.generateId());
    }
    return sa;
  }
  */
