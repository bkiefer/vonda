...
\todo{AW: It makes sense to mention this here, but it is kind of hidden here while warnings like this should be visible ''loud and clear'' in one location where a user might look when they're trying to find out what is going on with their system}
...
\todo{AW: Propose blocks are only frozen if the variables used are really final, thus maybe hint to that problem here}
...
\todo{AW: Just a mention that to my knowledge we never used behaviour timeouts in PAL, has it been tested?}
...
\vonda allows for using lambda constructions. At the moment, their usage is limited to the implementation of \texttt{Predicate} or \texttt{Comparator} in the functions listed in table \ref{tab:lambda-functions} that are pre-defined in the Agent framework.
...
\end{table}
\todo{AW: TODO: Update lambda list and resp sentence - should I cling to Predicate and Comparator as they are used here or use Function as the current Agent.rudi?}
...
This inclusion has two important effects. On one side, it triggers the compilation of the included file at exactly this point, such that any fields and methods known at this time will be available in the important file and it does not need to be compiled seperately. On the other side, it has the effect that all the rules contained in the imported file will be inserted in the rule cycle at the specific position of the \texttt{import}, where in the resulting code the \texttt{process()} method of the imported file will be executed.
...
So the \texttt{import} functionality makes it possible to distribute the rules of a project into multiple files, respectively modules, and pin them together in such a way that they are handled as one unit when compiling or executing the compilied code. This is not only useful for clarity and structuring of a project, but also supports modularity, as different subtrees of the \texttt{import} hierarchy can easily be added, moved, taken away or re-used in different projects.
...
\todo{AW: This all sounds good, but repeats part of what has been said before; while that might not be a bad thing, it is simply obvious that this text has been copy-pasted here from somewhere else (the paper) ...}
